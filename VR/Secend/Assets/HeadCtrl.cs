...
public class HeadCtrl : MonoBehaviour {
	...
	private void Turn(){
		...
		SteamVR_Controller.Device device = SteamVR_Controller.Input ((int) track.index);
		...
		//监听用户按键事件W
		if(Input.GetKey(KeyCode.W) || (isPressTouchPad && (angle >= -45f && angle <=45f))){
			_NextDir = HeadDir.Up;
			if(_CurrentDir == HeadDir.Down){
				_NextDir = _CurrentDir;
			}
		}
		if(Input.GetKey(KeyCode.S) || (isPressTouchPad && (angle <= -135f || angle >=135f))){
			_NextDir = HeadDir.Down;
			if(_CurrentDir == HeadDir.Up){
				_NextDir = _CurrentDir;
			}
		}
		if(Input.GetKey(KeyCode.A) || (isPressTouchPad && (angle <= -45f && angle >=-135f))){
			_NextDir = HeadDir.Left;
			if(_CurrentDir == HeadDir.Right){
				_NextDir = _CurrentDir;
			}
		}
		if(Input.GetKey(KeyCode.D) || (isPressTouchPad && (angle >= 45f && angle <= 135f))){
